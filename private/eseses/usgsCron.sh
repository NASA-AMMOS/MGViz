...
curl "https://earthquake.usgs.gov/fdsnws/event/1/query.geojson?starttime=`date --date '-30 years' +'%Y-%m-%d'`%2000%3A00%3A00&minmagnitude=5.95&orderby=time" -o ~/MGViz/Missions/MGViz/Layers/HistoricalSignificant.json
curl "https://earthquake.usgs.gov/fdsnws/event/1/query.geojson?starttime=`date --date '-30 years' +'%Y-%m-%d'`%2000%3A00%3A00&minmagnitude=5.50&maxmagnitude=5.95&orderby=time" -o ~/MGViz/Missions/MGViz/Layers/HistoricalModerate.json
